    ...
    while True:
        ...
        if command == 'q':
            ...
            try:
                ...
                copy_line_to_file(source_file, target_file, line_number)
            except ValueError:
                print("Номер строки должен быть числом.")

def copy_line_to_file(source_file, target_file, line_number):
    try:
        with open(source_file, "r", encoding='utf-8') as source:
            source_reader = DictReader(source)
            lines = list(source_reader)
            if line_number < 1 or line_number > len(lines):
                raise IndexError(f"Номер строки {line_number} вне диапазона файла {source_file}.")
            line_to_copy = lines[line_number - 1]

        with open(target_file, "a", encoding='utf-8', newline='') as target:
            target_writer = DictWriter(target, fieldnames=source_reader.fieldnames)
            if not exists(target_file):
                target_writer.writeheader()
            target_writer.writerow(line_to_copy)
